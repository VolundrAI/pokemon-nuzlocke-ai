from ..nuzlocke_utils import EncounterPool
from ..trainerai import GameAIPlayer, BestMovePlayer, GameAIDoublePlayer, BestMoveDoublePlayer
from ..utils import PokeCache
...
def team_build(model, encounter_pool, enemy_team) -> str:
    ...
    TODO: Use the playerai teambuilder to select the best team instead of a random team
    TODO: Update this logic to evolve pokemon based on level
    TODO: Update this logic to evolve pokemon based on item
    TODO: Update this logic to give held items to pokemon
    """
...
def find_dead_pokemon(our_name: str, replay_folder: str):
    """

    """

    f = sorted(glob(f"{replay_folder}/{our_name}*.html"))[-1]
    ...
    for line in str(battle_log).split("\n"):
        if "|faint|p1a" in line:
            dead = line.split(":")[1].strip().lower()
            dead_pokemon.append(dead)

    return dead_pokemon

async def do_run(run_df, battle_df, encounter_df, item_df, replay_path:str = "../../replays", cache: Optional[PokeCache] = None) -> bool:
    ...
    encounter_pool = EncounterPool(run_df, encounter_df, cache=cache)

    #Do each battle in order from start to finish
    for battle in battle_df.index:
        battle_id = battle_df.loc[battle, "BattleID"]
        battle_type = battle_df.loc[battle, "BattleType"]
        battle_required = battle_df.loc[battle, "Required?"]
        ...
        if battle_type.lower().strip() == "single":
            enemy_player = GameAIPlayer(player_configuration=enemy_player_config, server_configuration=server_config, max_concurrent_battles=25, battle_format="gen5anythinggoes", team=enemy_team)
            our_player = BestMovePlayer(player_configuration=our_player_config, save_replays=True, server_configuration=server_config, max_concurrent_battles=25, battle_format="gen5anythinggoes", team=our_team)
        elif battle_type.lower().strip() == "double":
            enemy_player = GameAIDoublePlayer(player_configuration=enemy_player_config, server_configuration=server_config, max_concurrent_battles=25, battle_format="gen5anythinggoesdoubles", team=enemy_team)
            our_player = BestMoveDoublePlayer(player_configuration=our_player_config, save_replays=True, server_configuration=server_config, max_concurrent_battles=25, battle_format="gen5anythinggoesdoubles", team=our_team)
        else:
            raise RuntimeError(f"{battle_type} is not a supported battle type")
        ...
        if our_player.n_won_battles == 1:
            ...
            dead_pokemon = find_dead_pokemon(our_name, replay_path)
            ...
            print("Remaining Pokemon:", [enc.pokemon.species for enc in encounter_pool.pool])
...
if __name__ == "__main__":
    ...
    asyncio.get_event_loop().run_until_complete(do_run(run_df, battle_df, encounter_df, item_df, cache_path))
